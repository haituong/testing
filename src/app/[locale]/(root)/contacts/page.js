...
const ContactPage = () => {
  ...
  const fetchTabs = async () => {
    ...
    try {
      const response = await fetch(
        "http://141.136.47.162:1347/api/views?sort[0]=createdAt:asc&filters[view_type][$eq]=contacts"
      );
      ...
      setTabItems(tabs);
      const savedActiveTab = localStorage.getItem("contact_activeTab");
      const savedActiveFilter = localStorage.getItem("contact_activeFilter");
  ...
  const handleTabChange = (key) => {
    ...
    localStorage.setItem(`contact_activeTab`, key);
    localStorage.setItem(`contact_activeFilter`, JSON.stringify(filters));
  };
  const onAddTab = async () => {
    ...
    try {
      ...
      const result = await response.json();
      localStorage.setItem(`contact_activeTab`, result.data.documentId);
      localStorage.setItem(
        `contact_activeFilter`,
  ...
  const onRemoveTab = async (key) => {
    ...
    try {
      ...
      if (key === activeTab && tabs.length > 0) {
        ...
        localStorage.setItem("contact_activeTab", firstTab.key);
        localStorage.setItem(
          "contact_activeFilter",
  ...
  const fetchData = async (params = {}, applyFilter = []) => {
    ...
    try {
      const response = await fetch(
        `http://141.136.47.162:1347/api/contacts${queryBuilder(
          applyFilter
        )}&pagination[page]=${
  ...
  const fetchColumns = async () => {
    ...
    const excludeColumns = ["notes", "tasks", "deals"];

    try {
      const response = await fetch(
        `http://141.136.47.162:1347/api/content-type-builder/content-types/api::contact.contact`
      );
      const result = await response.json();
  ...
  const handleTableColumn = () => {
    const savedSelection = JSON.parse(
      localStorage.getItem(`contactSelectedColumns`)
    );
  ...
  const fetchModuleStage = async () => {
    try {
      const response = await fetch(
        "http://141.136.47.162:1347/api/contact-statuses?sort[0]=position:asc"
      );
  ...
  const queryBuilder = (applyFilter = []) => {
    const prefix =
      "?populate[accounts][fields][0]=name&populate[contact_status][fields][0]=name&populate[contact_status][fields][1]=forecast_type&populate[contact_status][fields][2]=position&populate[contact_status][fields][3]=lifecycle_stage&populate[contact_status][fields][4]=stage_position&populate[tags][fields][0]=name&populate[tags][fields][1]=color&sort[0]=updatedAt:desc";
