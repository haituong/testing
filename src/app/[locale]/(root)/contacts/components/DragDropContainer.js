  ...
  const handleContainer = () => {
    try {
      ...
      dataSource.forEach((contact) => {
        const stage = contact.contact_status?.name;
        if (stagesMapped[stage]) {
          stagesMapped[stage].push(contact);
        }
      });
    ...
    if (sourceContainerKey && destinationContainerKey) {
      ...
      try {
        const response = await fetch(
          `http://141.136.47.162:1347/api/contacts/${draggedItem.documentId}`,
          {
            method: "PUT",
            headers: {
            ...
            body: JSON.stringify({
              data: {
                contact_status: destinationDocumentId, // Update the deal stage
              },
            }),
          }
        );
