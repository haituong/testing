...
export default function CreateComponent({
  ...
  const onFinish = async (values) => {
    ...
    const transformedData = {
      data: {
        ...
        contact_status: {
          connect: [
            {
              documentId: "hc6s8kp6yppowne5n5ix2atz", 
            },
          ],
        },
      },
    ...
    try {
      const response = await fetch(
        `http://141.136.47.162:1347/api/contacts`,
        {
          method: "POST",
      ...
      if (response.ok) {
        ...
        messageApi.open({
          type: "success",
          content: `Create contact successful!`,
        });
      } else {
      ...
      messageApi.error(`An error occurred while adding the contact.`);
    }
  };
