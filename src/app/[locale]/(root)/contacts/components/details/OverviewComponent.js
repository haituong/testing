...
export default function OverviewComponent({
  ...
  const fetchStages = async () => {
    try {
      ...
      setSelectedStage(
        detailData?.contact_status?.name || sortedStages[0]?.default?.name
      );
    } catch (error) {
  ...
  const saveNote = async (values) => {
    try {
      if (editingNoteId) {
        ...
        // Connect the newly created note to the contact
        ...
        if (!connectRes.ok) {
          throw new Error("Failed to connect the note to the contact");
        }
      }
  ...
  return (
    <div className="card">
            ...
            <div className="strip-container">
              {stages.map((stageGroup, index) => (
                <a
                  ...
                  className={`strip-stage ${getStageClass(stageGroup, index)} ${
      ...
      <Drawer
        ...
        open={childrenDrawer}
        footer={
          <Button type="primary" htmlType="submit" form={`contact_edit_form`}>
            Save
          </Button>
        }
      >
